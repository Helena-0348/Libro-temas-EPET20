...
export function useAdmin() {
  ...
  const colRef = useMemo(() => collection(db, "admin"), []);
  ...
  async function crearAdminConId(id, data) {
    const ref = doc(db, "admin", id);
    const payload = {
      activo: true,
      ...normAdmin(data),
  ...
  async function actualizarAdmin(id, data) {
    const ref = doc(db, "admin", id);
    const payload = { ...normAdmin(data), updatedAt: serverTimestamp() };
  ...
  async function desactivarAdmin(id) {
    await updateDoc(doc(db, "admin", id), { activo: false, updatedAt: serverTimestamp() });
  }

  // ------- ARRAYS: materias / divisiones -------
  async function agregarMateria(id, materia) {
    await updateDoc(doc(db, "admin", id), { materias: arrayUnion(materia), updatedAt: serverTimestamp() });
  }
  async function quitarMateria(id, materia) {
    await updateDoc(doc(db, "admin", id), { materias: arrayRemove(materia), updatedAt: serverTimestamp() });
  }
  async function agregarDivision(id, division) {
    await updateDoc(doc(db, "admin", id), { divisiones: arrayUnion(division), updatedAt: serverTimestamp() });
  }
  async function quitarDivision(id, division) {
    await updateDoc(doc(db, "admin", id), { divisiones: arrayRemove(division), updatedAt: serverTimestamp() });
  }
  ...
  // ------- BATCH IMPORT -------
  async function importarAdmin(arrayDeAdmin = []) {
    const batch = writeBatch(db);
    arrayDeAdmin.forEach((p) => {
      const ref = doc(collection(db, "admin"));
      batch.set(ref, {
        activo: true,
        ...normAdmin(p),
        createdAt: serverTimestamp(),
  ...
  // Para 100% robustez, usar un doc espejo /Admin_emails/{emailLower} con transacción o un Cloud Function.
  async function crearSiEmailUnico(data) {
    const existente = await findByEmail(data.email);
    if (existente) throw new Error("Ya existe un Admin con ese email.");
  ...
  async function crearConEmailUnicoTransaccional(data) {
    ...
    const mirrorRef = doc(db, "Admin_emails", emailLower);
    const profRef = doc(collection(db, "admin"));
    await runTransaction(db, async (tx) => {
      ...
      tx.set(mirrorRef, { AdminId: profRef.id, createdAt: serverTimestamp() });
    });
  }

  // ------- CAMPOS CONTADORES (ejemplo) -------
  async function incrementarCampo(id, campo = "cargas") {
    await updateDoc(doc(db, "admin", id), { [campo]: increment(1), updatedAt: serverTimestamp() });
  }
