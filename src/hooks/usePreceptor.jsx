...
export function usePreceprtores() {
  ...
  const colRef = useMemo(() => collection(db, "preceptores"), []);
  ...
  async function crearPreceprtor(data) {
    const payload = {
      activo: true,
      ...normPreceprtor(data),
  ...
  async function crearPreceprtorConId(id, data) {
    const ref = doc(db, "preceptores", id);
    const payload = {
      activo: true,
      ...normPreceprtor(data),
  ...
  async function actualizarPreceprtor(id, data) {
    const ref = doc(db, "preceptores", id);
    const payload = { ...normPreceprtor(data), updatedAt: serverTimestamp() };
    await updateDoc(ref, payload);
  }

  async function eliminarPreceprtor(id) {
    await deleteDoc(doc(db, "preceptores", id));
  }

  // Borrado lógico
  async function desactivarPreceprtor(id) {
    await updateDoc(doc(db, "preceptores", id), { activo: false, updatedAt: serverTimestamp() });
  }

  // ------- ARRAYS: materias / divisiones -------
  async function agregarMateria(id, materia) {
    await updateDoc(doc(db, "preceptores", id), { materias: arrayUnion(materia), updatedAt: serverTimestamp() });
  }
  async function quitarMateria(id, materia) {
    await updateDoc(doc(db, "preceptores", id), { materias: arrayRemove(materia), updatedAt: serverTimestamp() });
  }
  async function agregarDivision(id, division) {
    await updateDoc(doc(db, "preceptores", id), { divisiones: arrayUnion(division), updatedAt: serverTimestamp() });
  }
  async function quitarDivision(id, division) {
    await updateDoc(doc(db, "preceptores", id), { divisiones: arrayRemove(division), updatedAt: serverTimestamp() });
  }
  ...
  // ------- BATCH IMPORT -------
  async function importarPreceprtores(arrayDePreceprtores = []) {
    const batch = writeBatch(db);
    arrayDePreceprtores.forEach((p) => {
      const ref = doc(collection(db, "preceptores"));
      batch.set(ref, {
        activo: true,
        ...normPreceprtor(p),
        createdAt: serverTimestamp(),
  ...
  // Para 100% robustez, usar un doc espejo /Preceprtores_emails/{emailLower} con transacción o un Cloud Function.
  async function crearSiEmailUnico(data) {
    const existente = await findByEmail(data.email);
    if (existente) throw new Error("Ya existe un Preceprtor con ese email.");
    return crearPreceprtor(data);
  }
  ...
  async function crearConEmailUnicoTransaccional(data) {
    ...
    const mirrorRef = doc(db, "Preceprtores_emails", emailLower);
    const profRef = doc(collection(db, "preceptores"));
    await runTransaction(db, async (tx) => {
      ...
      tx.set(mirrorRef, { PreceprtorId: profRef.id, createdAt: serverTimestamp() });
    });
  }

  // ------- CAMPOS CONTADORES (ejemplo) -------
  async function incrementarCampo(id, campo = "cargas") {
    await updateDoc(doc(db, "preceptores", id), { [campo]: increment(1), updatedAt: serverTimestamp() });
  }
  ...
  return {
    ...
    // CRUD
    crearPreceprtor,
    crearPreceprtorConId,
    crearSiEmailUnico,
    crearConEmailUnicoTransaccional,
    actualizarPreceprtor,
    eliminarPreceprtor,
    desactivarPreceprtor,
